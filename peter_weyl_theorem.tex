...
\begin{enumerate}
    ...
    Nun betrachten wir $T:\LL^2(K)\to \LL^2(K), f\mapsto 
...
\item Die eigentlich spannende Eigenschaft ist, dass
    die Matrixkoeffizienten Punkte trennen. Zuerst 
    überlegen wir uns, dass es genügt zu zeigen, dass 
    es im Fall von topologischen Gruppen $G$ genügt, 
    dass es für alle $x \in G$ eine stetige 
    Funktion $f:G\to \C$, sodass $f(x)\neq f(1)$.\\
    Um dies zu sehen, seien $x_1 \neq x_2$. Dann gibt es
    nach Annahme eine stetige Funktion $f: G\to \C$,
    sodass $f( x_1^{-1} x_2) \neq f(1) $.  Dann ist
    ...
    Wir wollen also zeigen, dass es für alle $g\in G$ einen
    ...
    normaler Untergruppen normal ist, ist $H \unlhd G$.\\
    Damit erhalten wir auf $G/H$ eine wohldefinierte
    Gruppenstruktur.
\end{enumerate}
...
Lemma von Urysohn eine stetige Funktion $f:G\to \R$
...
stetige Funktion $f: K\to \R$, sodass $f(k)\neq f(1)$
...
Darüberhinaus kann jede stetige Funktion $f:\mathbb S^1\to
\C$ gleichmäßig durch trigonometrische Polynome angenähert
werden.
\end{satz}
