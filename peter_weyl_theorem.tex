...
\begin{itemize}
    ...
    \item Für zwei Banachräume $V, W$ bezeichnen wir mit
        $ \mathscr B(V, W)$ die Menge der beschränkten,
        stetigen linearen Abbildungen $T\colon V\to W$ und
	mit $ \mathscr B(V) = \mathscr B(V, V)$.
    ...
    \item Für einen topologischen Raum $X$ bezeichnen wir
        mit $C_0(X)$ die Menge der im Unendlichen
        verschwindenden, stetigen Funktionen.
\end{itemize}
\chapter{Grundlagen}
Dieses Kapitel dient lediglich als Zusammenfassung der 
wichtigsten Ergebnisse der Funktionalanalysis und Haarschen
Integrationstheorie, die in diesem Vortrag benötigt werden.
Da diese jedoch nicht das Hauptaugenmerk dieses Vortrags
sein sollen, wird teils auf Beweise (unter Angabe der 
entsprechenden Literatur) verzichtet.
...
\end{enumerate}
\end{proof}
Neben dem Haarmaß ist auch die Faltung ein wichtiges Werkzeug
in der harmonischen Analysis. 
\begin{definition}[Faltung]
Seien $f,g \in \LL^1(G)$. Dann definieren wir die
\underline{Faltung}\index{Faltung} von $f$ und $g$ als
die folgende Funktion:
$$ f\ast g(x) = \int_{G} f(y) g( y^{-1}x)  \,\diff y.$$
\end{definition}
Für dieses wollen wir die folgenden Eigenschaften ohne Beweis
festhalten. Beweise können beispielsweise \cite[Abschnitt
2.5]{abstractHarmAna} entnommen werden.
\begin{satz}\label{thm:propsConvolution}
\begin{enumerate}
    \item Die Faltung existiert für fast alle $x \in G$.
    \item Es gilt $||f \ast g||_1 \leq ||f||_1 ||g||_1$.
        Damit folgt, dass $ f \ast g \in \LL^1(G)$.
    \item Wenn $ f \in \LL^1(G), g \in \LL^ \infty(G)$, dann
        ist $ f \ast g$ stetig.
\end{enumerate}
\end{satz}
\begin{bemerkung}
Insbesondere erhalten wir aus der letzten Aussage von
\ref{thm:propsConvolution}, dass für $f \in \LL^1(G)$
und $ g \in C_0(G)$ auch $ f \ast g$ stetig ist, da
$ C_0(G) \subseteq \LL^\infty(G)$, da alle Funktionen,
die im Unendlichen verschwinden, beschränkt sind.
\end{bemerkung}
Zusätzlich benötigen wir den Begriff eines Dirac-Netzes.
...
\end{definition}
Auch für Dirac-Netze wollen wir die wichtigsten Eigenschaften
festhalten. 
\begin{satz}\label{thm:propsDiracNet}
\begin{enumerate}
    \item Es gibt stets ein Dirac-Netz für $G$.
    \item Sei $f \in \LL^2(G)$ und $ ( \phi_U)_U$ ein Dirac-Netz.
        Dann konvergiert $ f\ast \phi_U$ gegen $f$ in der 
        $\LL^2$-Topologie.
\end{enumerate}
\end{satz}
Die erste Aussage folgt aus dem Lemma von Urysohn. Die zweite Aussage
wird in \cite[Lemma 1.6.6]{principlesHarmAna} für $\LL^1$ bewiesen.
Für $\LL^2$ können wir dies analog unter Ausnutzung der 
Dreiecksungleichung für das Bochner-Integral definieren. Diese wird
beispielsweise in \cite{harmAnaBanachSpaces} bewiesen.
\section{Funktionalanalysis}
Neben der Haarschen Integrationstheorie ist ein weiteres wichtiges
Werkzeug die Funktionalanalysis. Insbesondere die Spektraltheorie
kompakter Operatoren wird ein wichtiges Werkzeug sein. Quelle dieses
Abschnitts ist \cite{functionalAnalysis}.
\subsection{Spektraltheorie kompakter Operatoren}
\begin{definition}[Kompakte Operatoren]
Eine lineare Abbildung $T$ zwischen normierten Räumen $X$ und
$Y$ heißt \underline{kompakt}\index{kompakt}, wenn $T(B_X)$ 
relativkompakt ist, d.h. $\overline{T(B_X)}$ ist kompakt. Die
Menge der kompakten Operatoren bezeichnet man mit $K(X, Y)$
und wir definieren $ K(X) \coloneq K(X, X)$.
\end{definition}
Auf einem separablen Hilbertraum $H$ kann man über eine wichtige 
Unterklasse der kompakten Operatoren reden:
\begin{definition}[Hilbert-Schmidt-Operatoren]
Ein Operator $T \in K(H)$ heißt \underline{Hilbert-Schmidt-Operator}
\index{Hilbert-Schmidt-Operator}, falls es eine Orthonormalbasis
$(e_n)_{n \in \N}$ gibt, sodass $ \sum_{n=1}^{\infty} \norm{Tg_n}^2
<\infty$ ist.
\end{definition}
Ein wichtiges Beispiel für Hilbert-Schmidt-Operatoren sind die 
sogenannten Integraloperatoren. Sei dazu $(X, \mathcal{A}, \mu)$
ein Maßraum.
\begin{beispiel}\label{ex:intOperatorsAreHilbertSchmidt}
	Eine Funktion $k \in \LL^2(X \times X)$ heißt ein 
	$\LL^2$-Kern. Für einen solchen Kern $k$ definieren wir
	auf $\LL^2(X)$ einen Operator
	$$K \phi(x) \coloneq \int_{X} k(x,y) \phi(y) \,\diff y.$$
	Diese Integral existiert fast überall in $x$ und für alle
	$ \phi \in \LL^2(X)$ gilt $ K \phi \in \LL^2(X)$. Damit
	erhalten wir einen Operator $K:\LL^2(X)\to \LL^2(X)$. 
	$K$ ist ein Hilbert-Schmidt-Operator.
\end{beispiel}
Einen Beweis dieses Ergebnis findet man in 
\cite[Proposition 5.3.3]{principlesHarmAna}.
Mit das wichtigste Werkzeug im Umgang mit kompakten Operatoren
ist der Spektralsatz für kompakte Operatoren.
\begin{satz}[Spektralsatz für kompakte Operatoren]
	\label{thm:spectralThm}
Sei $T \in K(H)$ normal (falls $\K = \C$) bzw. selbstadjungiert
(falls $\K = \R$). Dann existieren ein Orthonormalsystem
$e_1, e_2, \dots$ sowie eine Nullfolge $( \lambda_1, \lambda_2,
\dots) \subseteq \K \setminus \{ 0 \} $, sodass
$$ H = \ker(T) \oplus \overline{\vspan\{ e_1, e_2,\dots \}}$$
sowie
$$ Tx = \sum_{k} \lambda_k \langle x,e_k \rangle e_k$$
für alle $x \in H$.
\end{satz}
\subsection{Stetiger Funktionalkalkül}
\begin{satz}\label{thm:contFunctionalCalculus}
	Sei $ T \in \mathscr{B}(H)$ selbstadjungiert auf einem 
	Hilbertraum $H$. Dann existiert genau eine Abbildung
	$ \Phi: C( \sigma(T)) \to \mathscr{B}(H)$ mit
	\begin{enumerate}
		\item $ \Phi( \id) = T, \Phi( \mathbf{1}) = \id$,
		\item $ \Phi$ ist linear,
		\item $ \Phi$ ist multiplikativ, d.h.
			$ \Phi(f\cdot g) = \Phi(f)\circ \Phi(g)$
		\item $ \Phi$ ist involutiv, d.h. $ \Phi(\overline f)
			= \Phi(f)^*$,
		\item $ \Phi$ ist stetig.
	\end{enumerate}
	$ \Phi$ heißt der \underline{stetige Funktionalkalkül}
	\index{stetige Funktionalkalkül} von $T$ und wir schreiben
	$f(T) = \Phi(f)$ für $ f \in C( \sigma(T))$.
\end{satz}
Eine besondere Eigenschaft des Funktionalkalküls bezieht sich auf die
Situation, in der der Operator $T$, mit einem weiteren Operator
$S$ kommutiert. Dann gilt nämlich:
\begin{satz}\label{thm:funcCalcCommutes}
	Seien $T, S \in \mathscr{B}(H)$ mit $S \circ T = T \circ S$.
	Dann gilt für alle $ f \in C( \sigma(T))$, dass
	$f(T) \circ S = S \circ f(T)$.
\end{satz}
Diese Aussage findet man in \cite[Theorem 3]{specThmFromScratch}.
...
Sei $H$ ein Hilbertraum und $A \subseteq \mathscr B(H)$ eine
...
\begin{enumerate}
    \item Die einzige $A$-invariante, abgeschlossene Menge
        ungleich $ \{ 0 \} $ ist $ H$.
    \item Falls $T \in \mathscr B(H)$ mit allen $S \in A$
...
Angeommen (1) gilt. Sei also $T \in \mathscr B(H)$ ein 
...
\begin{enumerate}
    ...
    \item Für jede integrierbare Funktion $f$ gilt 
        $$ \int_{G} f( x^{-1}) \Delta( x^{-1})  \,\diff x
        = \int_{G} f(x) \,\diff x.$$
...
\begin{proof}
    \begin{enumerate}
        ...
        \item Sei $f \in C_c(G)$. Wir betrachten die
            Abbildung $$ I(f) = \int_{G} 
            f( x^{-1}) \Delta( x^{-1})\,\diff x.$$
            Dann gilt nach der ersten Eigenschaft, dass
            \begin{align*}
                I( L_z f) 
                &= \int_{G} f( z^{-1} x^{-1}) 
            \Delta( x^{-1})  \,\diff x\\
                &= \int_{G} f( (xz)^{-1}) \Delta( x^{-1}) 
                \,\diff x\\
                &= \Delta( z^{-1}) \int_{G} f( x^{-1})
                 \Delta( (x z^{-1})^{-1}\,\diff x\\
                &= \int_{G} f( x^{-1}) \Delta( x^{-1})  
                \,\diff x\\
                &= I(f)
            \end{align*}
            für alle $z \in G$, wobei im vorletzten Schritt
            verwendet wurde, dass $ \Delta$ ein
            Gruppenhomomorphismus ist. Damit ist $I$
            ein invariantes Integral. Somit gibt es ein
            $ c> 0$ mit $I(f) = c\int_{G} f(x) \,\diff x.$
            Wir wollen zeigen, dass $ c = 1$. Sei also
            $ \varepsilon >0$ und wähle eine symmetrische
            Umgebung der 1 $V$ (also $V = V^{-1}$) mit
            $|1- \Delta(s)|< \varepsilon$ (durch Stetigkeit
            von $ \Delta$) für alle $ s \in V$. Wähle
            eine symmetrische Funktion $f \in C_c^+(V)$
            mit $f \neq 0$. Dann gilt:
            \begin{align*}
            |1-c| \int_{G} f(x) \,\diff x
            &= \left| \int_{G} f(x) \,\diff - I(f)\right|\\
            &\leq \int_{G} |f(x)-f(x^{-1})\Delta( x^{-1})|
            \,\diff x\\
            &= \int_{V} f(x)|1- \Delta( x^{-1})|\,\diff x\\
            &< \varepsilon \int_{G} f(x) \,\diff x.
            \end{align*}
            Da $ f\neq 0$, folgt $|1-c|<\varepsilon$ für 
            alle $\varepsilon >0$. Damit folgt $c=1$ und
...
\begin{enumerate}
    ...
    \item Nun betrachten wir den Fall, dass wir den
        ...
        \begin{align*}
            ...
            &= \lambda_i
        \end{align*}
        wobei im vorletzten Schritt Satz 
        \ref{thm:propsModularFunction}
...
\begin{enumerate}
    ...
    Mit Satz \ref{thm:propsConvolution} und, da es ein $U \in
    ...
    Aus Beispiel \ref{ex:intOperatorsAreHilbertSchmidt} folgt, 
    dass es sich bei $T = T^* \neq 0$ (nicht 0,
    ...
    Aus dem \hyperref[thm:spectralThm]{Spektralsatz für kompakte 
...
\href{https://math.stackexchange.com/questions/1479222/peter-weyl-theorem-versions}{\textcolor{blue}{Peter-Weyl theorem versions}}.}. 
    ...
    Funktion $f\colon K\to \C$, sodass $f(x)\neq f(1)$.\\
...
folgt, dass sich jedes $ f \in C(\SB^1)$ als Grenzwert von
Funktionen der Form $\sum_{i=1}^{n} c_ {i } z^i$, 
mit $c_ { n_i } \in \C$.
Hier wird die Konvergenz im Sinne der 
Supremumsnormkonvergenz verstanden und damit handelt es
sich um gleichmäßige Konvergenz, was die Aussage zeigt.
\end{proof}
\chapter{Ausblick}
In diesem Vortrag wurde eine Version des Satzes von
Peter-Weyl bewiesen, welche die gängigste Formulierung
des \hyperref[thm:peterWeyl]{Satzes von Peter-Weyl} ist. 
Aus diesem Satz folgen jedoch noch weitere spannende 
Ergebnisse. Aus Zeitgründen kann auf diese im Vortrag nicht
eingegangen werden. Der Vollständigkeit sollen diese 
dennoch erwähnt werden. Die Beweise finden sich in
\cite{principlesHarmAna}. Zuerst wollen wir jedoch eine 
spannende Konsequenz des \hyperref[thm:peterWeyl]
{Satzes von Peter-Weyl} geben:
\begin{satz}
Sei $K$ eine kompakte, topologische Gruppe. Dann gilt
\begin{enumerate}
    \item $\UDual{K} = \UDualFin{K}$, also sind alle
        irreduziblen, unitären Darstellungen 
        endlichdimensional.
    \item Jede unitäre Darstellung von $K$ ist die orthogonale
        Summe von irreduziblen Darstellungen.
\end{enumerate}
\end{satz}
Nun formulieren wir noch ein weiteres Ergebnis, welches
als Satz von Peter-Weyl bezeichnet werden:
\begin{satz}
Es gibt einen unitären Isomorphismus
$$ \LL^2(K) \cong \widehat{ \bigoplus}_ { \tau \in \hat K }
\End( V_ \tau),$$
welcher ein Vertauscher bzgl. der Darstellung $ \eta
= \oplus_ { \tau \in \hat K } \eta_ \tau $ mit
$ \eta_ \tau(k_1, k_2)(T) = \tau(k_1)T \tau( k_2^{-1})$
für $k_1, k_2 \in K$ und $T \in\End(V_ \tau)$.\\
Dieser Isomorphismus bildet ein $ f \in \LL^2(K) \subseteq
\LL^1(K)$ auf $ \sum_{ \tau \in \hat K} \tau(f)$ ab, wobei
$$ \tau(f) = \int_{K} f(x) \tau(x) \,\diff x.$$
Insbesondere definieren wir für $f \in \LL^2(K)$ die
Abbildung $ \hat f : \hat K\to \hat{ \bigoplus}
_ { \tau \in \hat K } \End( V_ \tau)$ durch
$ \hat f( \tau) = \tau(f)$. Für diese gilt $\norm f
= \norm{ \hat f}$ für alle $f \in \LL^2(K)$. 
\end{satz}
Somit erhalten wir durch den Satz von Peter-Weyl einen
Plancherel-Satz auf kompakten Gruppen $K$.\\
