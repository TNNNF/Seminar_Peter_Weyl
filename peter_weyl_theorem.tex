...
$$ \rho_L: G\to \GLS{\LL^2(G)}, x\mapsto L_x$$
...
erhalten wir mittels $\rho\otimes \tau: K\to 
...
zeigen, dass es sich beim Spann der $ \tau$
um Matrixkoeffizienten um eine Algebra handelt,
...
\item Wir betrachten die linksreguläre Darstellung 
	$ \rho_L$. Nach \ref{thm:peterWeyl} ist ein 
	direktes Produkt von endlichdimensionalen, 
	irreduziblen Darstellungen und somit in obigem 
	$M$. Zudem istdiese Darstellung auch treu. Seien 
	nun $ g\neq h \in G$. Nach obigen Überlegungen
	zur linksregulären Darstellung muss 
	$ \rho_L(g)\neq \rho_L(h)$ sein, da $ \rho_L$
	injektiv ist. Insbesondere sind $ \rho_L(g)$
	und $ \rho_L(h)$ stetige Operatoren zwischen
	Hilberträumen. Damit sind diese gleich genau
	dann, wenn ihre Matrixkoeffizienten gleich sind.
	Damit muss es $v,w \in \LL^2(K)$ geben,
	sodass $$ \langle \rho_L(g)v, w \rangle
	\neq \langle \rho_L(h)v,w \rangle.$$
	Dies sind die punktetrennenden 
	Matrixkoeffizienten, deren Existenz wir beweisen
	wollten.
\end{enumerate}
